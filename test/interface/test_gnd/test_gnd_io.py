...
class TestGNDio(unittest.TestCase):
    ...
    def test_Cache_get_gndids_of_beacon_file(self):
        for fr in self.get_FileReaders():
            if ".txt" in fr.filepath:
                c = Cache(fr.loadfile_beacon(), False)
                result = c.get_gndids_of_beacon_file()
                self.assertEqual(type(result), list, "get_gndids_of_beacon_file() should return a list")
                self.assertGreater(len(result), 0, "get_gndids_of_beacon_file() should return a list with elements in it")
