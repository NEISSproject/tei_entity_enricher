...
class Identifier:
    ...
    def get_wikidata_search_results(
        ...
        if self.input == None:
            print(
                "Identifier.get_wikidata_search_results() Error: Identifier has no input data."
        ...
        if (
            ...
            print(
                "Identifier.get_wikidata_search_results() Error: Identifier input data is in a wrong format."
            ) if self.show_printmessages == True else None
            return False
        result_dict = {}
        for string_tuple in self.input:
            filereader = FileReader(
                "https://www.wikidata.org/w/api.php?action=wbsearchentities&search={}&format=json&language=de".format(
                    string_tuple[0]
                ),
                "web",
                True,
            ...
            if (
                not filter_for_precise_spelling == True
                and not filter_for_correct_type == True
            ):
                print(
