...
class Connector:
    def __init__(
        ...
        self.apilist: Union[dict, None] = FileReader(
            os.path.join(module_path, "util", "apilist.json"), "local", True
        ).loadfile_json()
        if self.apilist is None:
            print(
                "connector error: could not find apilist.json. using default settings..."
        ...
        if self.check_connectivity == True:
            ...
            print(
                "connector: initialization has been done without connectivity check."
            ) if self.show_printmessages else None

    def connectivitycheck_single(
        self, index_to_test: int, gnd_to_test: str = "118540238"
    ) -> bool:
        ...
        response: requests.models.Response = requests.get(
            self.apilist[index_to_test]["baseUrl"].format(gnd_to_test)
        )
    ...
    def connectivitycheck_loop(self) -> int:
        ...
        if len(self.remaining_apis_to_check) > 0:
            if self.connectivitycheck_single(self.remaining_apis_to_check[0]) == True:
                print(
                    "connectivity check passed: connection to {} api established.".format(
                        self.apilist[self.remaining_apis_to_check[0]]["name"]
                    )
                ) if self.show_printmessages else None
                ...
                print(
                    "connectivity check: {} api is currently not responding as expected. checking for alternatives...".format(
                        self.apilist[self.remaining_apis_to_check[0]]["name"]
                    )
                ) if self.show_printmessages else None
            ...
            print(
                "connectivity check error: none of the listed apis is responding as expected."
            ) if self.show_printmessages else None
            return -1
    ...
    def print_complete_url(self, index: int = 0) -> int:
        ...
        if self.apiindex not in [i for i, _ in enumerate(self.apilist)]:
            print(
                "connector.print_complete_url() error: apiindex is not defined correctly. using default api..."
            ) if self.show_printmessages else None
            self.apiindex = 0
        if self.gnd is not None:
            if type(self.gnd) == str:
                print(
                    "connector complete URL: {}".format(
                        self.apilist[self.apiindex]["baseUrl"].format(self.gnd)
                    )
                ) if self.show_printmessages else None
            elif type(self.gnd) == list:
                print(
                    "connector complete URL of gnd number {} in passed gnd list: {}".format(
                        index + 1,
                        self.apilist[self.apiindex]["baseUrl"].format(self.gnd[index]),
                    )
                ) if self.show_printmessages else None
            return 0
        else:
            print(
                "connector error in print_complete_url(): no gnd number has been passed to connector object yet."
            ) if self.show_printmessages else None
            return -1
    ...
    def return_complete_url(self, index: int = 0) -> Union[str, None]:
        ...
        if self.apiindex not in [i for i, _ in enumerate(self.apilist)]:
            print(
                "connector.return_complete_url() error: apiindex is not defined correctly. using default api..."
            ) if self.show_printmessages else None
            self.apiindex = 0
        if self.gnd is not None:
            if type(self.gnd) == str:
            ...
            print(
                "connector error in return_complete_url(): no gnd number has been passed to connector object yet."
            ) if self.show_printmessages else None
            return None
    ...
    def get_gnd_data(
        ...
        if self.check_connectivity == False:
            print(
                "connector note: connections to apis have not been checked yet. to do so manually execute connectivitycheck_loop() method of the current connector object. continuing attempt to receive gnd data from {} api...".format(
                    self.apilist[self.apiindex]["name"]
                )
            ) if self.show_printmessages else None
        elif self.connection_established == False:
            print(
                "connectivity error: after connectivity check no connection could has been established to any of the available apis. gnd data queries can not be executed at the moment."
            ) if self.show_printmessages else None
            return None
        result = {}
        if type(self.gnd) == str:
            try:
                ...
                print(
                    "connectivity error in get_gnd_data() method: could not load resource from api as expected."
                ) if self.show_printmessages else None
                return None
            self.connection_established = True
            if response.status_code == 200:
                ...
                response.close()
                print(
                    "connector.get_gnd_data() status: data for gnd {} received.".format(
                        self.gnd
                    )
                ) if self.show_printmessages else None
            else:
                print(
                    "connector.get_gnd_data() status: for gnd {} no data could be delivered by api".format(
                        self.gnd
                    )
                ) if self.show_printmessages else None
                return None
            ...
            for index, gnd in enumerate(self.gnd):
                ...
                if response.status_code == 200:
                    ...
                    response.close()
                    print(
                        "connector.get_gnd_data() status: gnd {} ({}) of {} processed".format(
                            index + 1, gnd, len(self.gnd)
                        )
                    ) if self.show_printmessages else None
                else:
                    print(
                        "connector.get_gnd_data() status: for gnd {} ({}) of {} no data could be delivered by api".format(
                            index + 1, gnd, len(self.gnd)
                        )
                    ) if self.show_printmessages else None
            self.connection_established = True
        ...
        def filter_received_data(gnd: str, mode: Union[str, List[str]]) -> dict:
            ...
            for category in selected_categories:
                _temp_data = []
                try:
                    _temp_data = result[gnd][
                        self.apilist[self.apiindex]["baseAliases"][category][0]
                    ]
                except KeyError:
                    _temp_data = []
                    print(
                        "connector.get_gnd_data() filtering note: could not find {} information for {} in raw data. continuing processing...".format(
                            category, gnd
                        )
                    ) if self.show_printmessages else None
