...
class FileReader:
    ...
    def loadfile_json(self) -> Union[dict, str, None, bool]:
        ...
        if self.filepath == None:
            print(
                "internal error: FileReader.filepath not defined"
            ) if self.show_printmessages else None
            return False
        if self.origin == None:
            print(
                "internal error: FileReader.origin not defined"
            ) if self.show_printmessages else None
            return False
            ...
            try:
                ...
                print(
                    "error: file not found or bad format"
                ) if self.show_printmessages else None
                return None
    ...
    def loadfile_beacon(self) -> Union[str, None, bool]:
        ...
        if self.origin == "local":
            ...
            try:
                ...
                print(
                    "error: couldn't get data due to connection or filepath issue"
                ) if self.show_printmessages else None
                return None
...
class Cache:
    ...
    def check_for_redundancy(
        self, gnd: str, category: str, value: Union[str, dict, list, bool, None]
    ) -> Tuple[bool, bool]:
    ...
    def get_gndids_of_beacon_file(self) -> List[str]:
        ...
        if self.check_beacon_prefix_statement() == True:
            ...
            print(
                "error: loaded beacon-file doesn't refer to gnd data or is corrupted"
            ) if self.show_printmessages else None
            return None
...
class FileWriter:
    ...
    def writefile(self, do_if_file_exists: str = "cancel") -> bool:
        ...
        def do_if_file_exists_cancel() -> bool:
            print(
                "file already exists: cancel writing process"
            ) if self.show_printmessages else None
            return False
        ...
        def do_if_file_exists_replace() -> bool:
            ...
            print(
                "file already exists: file successfully overwritten"
            ) if self.show_printmessages else None
            return True
        ...
        def do_if_file_exists_merge() -> bool:
            if already_existing_file_cache.check_json_structure() == False:
                print(
                    "file already exists: couldn't merge files due to incompatibility issue"
                ) if self.show_printmessages else None
                return False
            else:
                for key in self.data:
                    ...
                    if any(redundancy_check_result):
                        print(
                            "file already exists: couldn't merge files due to redundancy issue"
                        ) if self.show_printmessages else None
                        return False
                ...
                print(
                    "file already exists: files successfully merged"
                ) if self.show_printmessages else None
                return True
        ...
        if already_existing_file_cache.data == None:
            with open(self.filepath, "w") as file:
                json.dump(self.data, file, indent="\t")
            print(
                "new file {} successfully created".format(self.filepath)
            ) if self.show_printmessages else None
            return True
            ...
            with open(self.filepath, "w") as file:
                json.dump(self.data, file, indent="\t")
            print(
                "file already exists, but was empty: file successfully written"
            ) if self.show_printmessages else None
            return True
        elif already_existing_file_cache.data == False:
            print(
                "internal error: cancel writing process"
            ) if self.show_printmessages else None
            return False
        else:
