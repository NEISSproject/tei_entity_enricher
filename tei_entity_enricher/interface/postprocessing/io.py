...
class FileReader:
    ...
    def loadfile_json(self) -> Union[dict, str, None, bool]:
        ...
        if self.filepath == None:
            print("FileReader loadfile_json() internal error: FileReader.filepath not defined") if self.show_printmessages else None
            return False
        if self.origin == None:
            print("FileReader loadfile_json() internal error: FileReader.origin not defined") if self.show_printmessages else None
            return False
            ...
            try:
                ...
                print("FileReader loadfile_json() error: file not found or bad format") if self.show_printmessages else None
                return None
    def loadfile_beacon(self) -> Union[str, None, bool]:
        ...
        the method can return:
            ...
            or False (in case of file not found error or bad format error)"""
        if self.filepath == None:
            print("FileReader loadfile_beacon() internal error: FileReader.filepath not defined") if self.show_printmessages else None
            return False
        if self.origin == None:
            print("FileReader loadfile_beacon() internal error: FileReader.origin not defined") if self.show_printmessages else None
            return False
        if self.origin == "local":
            ...
            try:
                ...
                print("FileReader loadfile_beacon() error: couldn't get data due to connection or filepath issue") if self.show_printmessages else None
                return None
...
class Cache:
    ...
    def get_gnd_ids_of_beacon_file(self) -> List[str]:
        ...
        if self.check_beacon_prefix_statement() == True:
            ...
            print(f"in beacon found gndids: {len(result_list)}\ndata: {result_list}") if self.show_printmessages else None
            return result_list
        else:
            print("Cache get_gnd_ids_of_beacon_file() error: loaded beacon-file doesn't refer to gnd data or is corrupted") if self.show_printmessages else None
            return None
...
class FileWriter:
    def __init__(self, \
        ...
        self.filepath: Union[str, None] = filepath
        self.show_printmessages: bool = show_printmessages
        self.writefile_types: dict = {
            ".json": "writefile_json"
        }
    def writefile_json(self, \
        ...
        def do_if_file_exists_cancel() -> bool:
            print("FileWriter writefile_json(): file already exists, cancel writing process") if self.show_printmessages else None
            return False
        def do_if_file_exists_replace() -> bool:
            with open(self.filepath, "w") as file:
                json.dump(self.data, file, indent="\t")
            print("FileWriter writefile_json(): file already exists, file successfully overwritten") if self.show_printmessages else None
            return True
        def do_if_file_exists_merge() -> bool:
            if already_existing_file_cache.check_json_structure() == False:
                print("FileWriter writefile_json(): file already exists, couldn't merge files due to incompatibility issue") if self.show_printmessages else None
                return False
            else:
                for key in self.data:
                    ...
                    if any(redundancy_check_result):
                        print("FileWriter writefile_json(): file already exists, couldn't merge files due to redundancy issue") if self.show_printmessages else None
                        return False
                already_existing_file_cache.data.update(self.data)
                with open(self.filepath, "w") as file:
                    json.dump(already_existing_file_cache.data, file, indent="\t")
                print("FileWriter writefile_json(): file already exists, files successfully merged") if self.show_printmessages else None
                return True
        ...
        if already_existing_file_cache.data == None:
            with open(self.filepath, "w") as file:
                json.dump(self.data, file, indent="\t")
            print(f"FileWriter writefile_json(): new file {self.filepath} successfully created") if self.show_printmessages else None
            return True
        elif already_existing_file_cache.data == "empty":
            with open(self.filepath, "w") as file:
                json.dump(self.data, file, indent="\t")
            print("FileWriter writefile_json(): file already exists but was empty, file successfully written") if self.show_printmessages else None
            return True
        elif already_existing_file_cache.data == False:
            print("FileWriter writefile_json() internal error: cancel writing process") if self.show_printmessages else None
            return False
        else:
