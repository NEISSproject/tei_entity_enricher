...
class TestPostprocessingWikidataConnector(unittest.TestCase):
    ...
    def test_init(self):
        for connector in self.get_connectors():
            self.assertIsInstance(
                connector,
                WikidataConnector,
                "variable con should refer to an instance of WikidataConnector class",
            )
            self.assertEqual(
                type(connector.input[0]),
                tuple,
    ...
    def test_connectivity_check(self):
        for connector in self.get_connectors():
            self.assertEqual(
                type(connector.connectivity_check()),
                int,
                "connectivity_check() should return integer value",
            )
    ...
    def test_get_wikidata_search_results(self):
        for connector in self.get_connectors():
            ...
            self.assertIsInstance(
                result,
                dict,
                "return value type of get_wikidata_search_results() should be dict",
            )
            self.assertIsInstance(
                result[list(result.keys())[0]],
                list,
                "from get_wikidata_search_results() returned dict should have lists as its values",
            )
            self.assertIsInstance(
                result[list(result.keys())[0]][0],
                int,
                "from get_wikidata_search_results() returned lists in dict should have an integer value on index 0",
            )
            self.assertIsInstance(
                result[list(result.keys())[0]][1],
                dict,
                "from get_wikidata_search_results() returned lists in dict should have an dict value on index 1",
            )
