...
class TestPostprocessingIdentifier(unittest.TestCase):
    ...
    def test_query(self):
        for identifier in self.get_identifiers():
            ...
            self.assertIsInstance(result[list(result.keys())[0]], list, "from query() returned dict should have lists as its values")
            self.assertIsInstance(result[list(result.keys())[0]][0], int, "from query() returned lists in dict should have an integer value on index 0")
            self.assertIsInstance(result[list(result.keys())[0]][1], dict, "from query() returned lists in dict should have an dict value on index 1")
