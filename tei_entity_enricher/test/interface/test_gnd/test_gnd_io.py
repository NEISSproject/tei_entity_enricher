...
class TestGNDio(unittest.TestCase):
    ...
    def get_FileReaders(self):
        return [
            ...
            FileReader(
                os.path.join(module_path, "util", "apilist.json"), "local", False, False
            ),
        ]
        ...
        for fr in self.get_FileReaders():
            if ".txt" in fr.filepath:
                ...
                self.assertEqual(
                    type(result),
                    list,
                    "get_gndids_of_beacon_file() should return a list",
                )
                self.assertGreater(
                    len(result),
                    0,
                    "get_gndids_of_beacon_file() should return a list with elements in it",
                )
    ...
    def test_FileWriter_write_file(self):
        ...
        if os.path.exists(os.path.join(module_path, "util", "testfile.json")):
            print(
                "test_FileWriter_write_file(): testfile.json already exists. Delete and continue test? (Y or skip with everything else)"
            )
            answer = input()
            if answer == "Y":
        ...
        self.assertTrue(
            fw.writefile(), "writefile() should return True, when writing a new file"
        )
        self.assertFalse(
            fw.writefile(),
            "writefile() should return False, when a file with filepath already exists and writefile()s parameter 'do_if_file_exists' is 'cancel'",
        )
        self.assertTrue(
            fw.writefile("replace"),
            "writefile() should return True, when replacing an existing file and writefile()s parameter 'do_if_file_exists' is 'replace'",
        )
        fw.data = {
        ...
        self.assertTrue(
            fw.writefile("merge"),
            "writefile() should return True, when merging data with an existing file and writefile()s parameter 'do_if_file_exists' is 'merge'",
        )
        if os.path.exists(os.path.join(module_path, "util", "testfile.json")):
            os.remove(os.path.join(module_path, "util", "testfile.json"))
