    ...
    def test_init(self):
        for con in self.get_connectors():
            self.assertIsInstance(
                con,
                Connector,
                "variable con should refer to an instance of Connector class",
            )
            self.assertEqual(
    ...
    def test_return_url(self):
        for con in self.get_connectors():
            self.assertEqual(
                ...
                con.apilist[con.apiindex]["baseUrl"].format(con.gnd[0]),
                "return_complete_url() should return a string constructed of 'baseUrl' value (first object in apilist) and gnd '118629662'",
            )
