...
class SparQLDef:
    ...
    def init_sparql_list(self):
        ...
        with open(self.sds_path) as f:
            self.sparqldict = json.load(f)
        self.check_reload()
        self.editable_sparql_names = [
            sparql_name for sparql_name in self.sparqldict if not self.sparqldict[sparql_name][2]
        ]
            ...
            if self.ntd.ntd_attr_sparql_map in definition.keys():
                for entity in definition[self.ntd.ntd_attr_sparql_map].keys():
                    if definition[self.ntd.ntd_attr_sparql_map][entity] == name:
                        val = False
                        st.error(
                            f"Deletion of the SparQL Query {name} is not allowed, because it is already used in the {menu_entity_definition} {definition[self.ntd.ntd_attr_name]}. If necessary first remove the assignment of the SparQL Query from the {menu_entity_definition}."
                        )
        return val
    ...
    def show_editable_sparql_content(self, mode):
        if mode == self.sds_mode_edit and len(self.editable_sparql_names) < 1:
            st.info(
                f"There are no self-defined {menu_sparql_queries} to edit in the moment. If you want to edit a template you have to duplicate it."
            )
        else:
            ...
            def save_sds(cur_name, cur_content, mode):
                ...
                st.session_state.sds_save_message = f"SparQL Query {cur_name} succesfully saved!"
    ...
    def sparql_del(self):
        def delete_sds(sparql_name):
            ...
            st.session_state.sds_save_message = f"SparQL Query {sparql_name} succesfully deleted!"
        ...
        if len(self.editable_sparql_names) > 0:
            ...
            if self.validate_definition_for_delete(
                st.session_state.sds_sel_del_ql_name, self.sparqldict[st.session_state.sds_sel_del_ql_name]
            ):
                st.button(
    ...
    def show_edit_environment(self):
        sds_definer = st.expander(f"Add or edit existing {menu_sparql_queries}", expanded=False)
        with sds_definer:
    ...
    def show_sds(self):
        ...
        with sds_show:
            ...
            with col2:
                st.selectbox(
                    label="Choose a SparQL Query for showing its details:",
                    options=list(self.sparqldict.keys()),
                    key="sds_show_query_name",
                )
    ...
    def show(self):
        st.latex("\\text{\Huge{" + menu_sparql_queries + "}}")
        self.show_sds()
