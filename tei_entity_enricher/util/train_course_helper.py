    ...
    for line in lines:
        ...
        if result_start_index >= 0:
            try:
                epoch = int(
                    line[
                        result_start_index + len("Results of epoch") : result_start_index + len("Results of epoch") + 5
                    ].replace(" ", "")
                )
...
def show_metric_line_chart(metrics, metric_name):
    ...
    alt_chart_data = pd.DataFrame({c_epoch: epochlist, metric_name: valuelist})
    ...
    alt_chart = (
        ...
        .mark_line()
        .encode(x=c_epoch, y=alt.Y(metric_name, scale=alt.Scale(domain=[min_scale, max_scale])))
    )
    st.altair_chart(alt_chart)


def extract_evaluations_to_model_path(modelpath):
    eval_dict_list = []
    for file in os.listdir(os.path.join(modelpath, "best")):
        if file.startswith("lav_results_test_") and file.endswith(".lst.json"):
            with open(os.path.join(modelpath, "best", file)) as f:
                eval_json = json.load(f)
            if "metrics" in eval_json.keys() and "SeqEvalF1FixRule" in eval_json["metrics"].keys():
                eval_dict_list.append({c_tng: file[17:-9], c_ef1: eval_json["metrics"]["SeqEvalF1FixRule"]})
