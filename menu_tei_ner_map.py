...
class Menu_ner_tei_map():
    ...
    def validate_and_saving_mapping(self, mapping, mode):
        val=True
        if self.tnm_attr_name not in mapping.keys() or mapping[self.tnm_attr_name] is None or mapping[self.tnm_attr_name]== '':
            val=False
            st.error('Please define a name for the mapping before saving!')
        elif os.path.isfile(os.path.join(self.tnm_Folder, mapping[self.tnm_attr_name].replace(' ', '_') + '.json')) and mode!=self.tnm_mode_edit:
            val=False
        ...
        if val:
            ...
            with open(os.path.join(self.tnm_Folder, mapping[self.tnm_attr_name].replace(' ', '_') + '.json'), 'w+') as f:
                json.dump(mapping, f)
    ...
    def show_editable_mapping_content(self,mode):
        if mode==self.tnm_mode_edit and len(self.editable_mapping_names)<1:
            st.info('There are no self-defined TEI NER Entity Mappings to edit in the moment. If you want to edit a template you have to duplicate it.')
        else:
